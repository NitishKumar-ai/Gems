// Gems — shared capture core.
//
// This module holds everything capture needs and knows nothing about *how* it was triggered.
// Two entrypoints sit on top of it:
//
//   - `captureOne` — one session, located from a `{session_id, cwd}` payload. This is what the
//     Claude Code `SessionEnd` hook drives (see `../hooks/capture-session.mjs`).
//   - `captureAll` — scan every known CLI's transcript directory and capture whatever is new.
//     This is `gems capture`, and it is what lets Gems work on a CLI with no hook system at all:
//     the transcripts are already on disk, so capture can read them on demand instead of waiting
//     for a lifecycle event that only Claude Code emits.
//
// Both funnel through `captureTranscript`, so the redaction boundary, the dedupe rule, and the
// `0600` store permissions are defined in exactly one place. The derived record holds counts,
// ratios, model ids and timestamps only — no prompt text, file content, or command output is ever
// read into it (that boundary lives in `./extract.mjs`).

import {
  appendFileSync,
  closeSync,
  createReadStream,
  existsSync,
  mkdirSync,
  openSync,
  readdirSync,
  readFileSync,
  readSync,
  statSync,
  writeFileSync,
} from 'node:fs';
import { homedir } from 'node:os';
import { basename, join } from 'node:path';

import { extractTranscript } from './extract.mjs';

// 2 adds the derived `metrics` object. A schema 1 record is a valid, metric-less session;
// readers treat a missing `metrics` as thin history rather than as zeroes.
export const SCHEMA_VERSION = 2;

// This store names repositories, working directories, transcript paths, session ids and
// activity times. On a shared machine a default umask would let other accounts read it.
const OWNER_ONLY_DIR = 0o700;
const OWNER_ONLY_FILE = 0o600;

const MAX_DIAGNOSTIC_LINES = 500;
// Session transcripts are line-delimited JSON and grow with conversation length.
// Past this size we record the session from stat alone rather than walk it — the hook
// has a 10s timeout, and an external kill skips this script's own error handling.
export const MAX_LINE_COUNT_BYTES = 32 * 1024 * 1024;
// Dedupe only needs the recent tail of the store, not its whole history.
const DEDUPE_TAIL_BYTES = 64 * 1024;

export function gemsHome(env = process.env) {
  return env.GEMS_HOME || join(homedir(), '.gems');
}

export function claudeProjectsDir(env = process.env) {
  return env.GEMS_CLAUDE_PROJECTS || join(env.CLAUDE_CONFIG_DIR || join(homedir(), '.claude'), 'projects');
}

export function codexSessionsDir(env = process.env) {
  return env.GEMS_CODEX_SESSIONS || join(homedir(), '.codex', 'sessions');
}

export function cursorSessionsDir(env = process.env) {
  return env.GEMS_CURSOR_SESSIONS || join(homedir(), '.cursor', 'sessions');
}

export function agySessionsDir(env = process.env) {
  return env.GEMS_AGY_SESSIONS || join(homedir(), '.gemini', 'antigravity-cli', 'brain');
}

export function windsurfSessionsDir(env = process.env) {
  return env.GEMS_WINDSURF_SESSIONS || join(homedir(), '.windsurf');
}

export function aiderSessionsDir(env = process.env) {
  return env.GEMS_AIDER_SESSIONS || process.cwd();
}

export function copilotSessionsDir(env = process.env) {
  return env.GEMS_COPILOT_SESSIONS || join(homedir(), '.copilot');
}

/**
 * The transcript sources Gems understands, in priority order. `captureOne` walks this list to
 * decide which CLI a given session id belongs to; `captureAll` enumerates each one. Adding a CLI
 * is a matter of a directory resolver plus a normalizer in `./normalizers/` — nothing here is
 * Claude-specific beyond the first entry's default path.
 *
 * `layout` tells `enumerateSessions` how that CLI lays transcripts out on disk:
 *   - `claude`: `<projectSlug>/<sessionId>.jsonl` (nested), or flat in the root
 *   - `flat`:   `<sessionId>.jsonl` directly in the directory
 *   - `agy`:    `<sessionId>/.system_generated/logs/transcript[_full].jsonl` (nested)
 */
export function sources(env = process.env) {
  return [
    { name: 'claude-code', dir: claudeProjectsDir(env), layout: 'claude' },
    { name: 'codex', dir: codexSessionsDir(env), layout: 'flat' },
    { name: 'cursor', dir: cursorSessionsDir(env), layout: 'flat' },
    { name: 'agy', dir: agySessionsDir(env), layout: 'agy' },
    { name: 'windsurf', dir: windsurfSessionsDir(env), layout: 'flat' },
    { name: 'aider', dir: aiderSessionsDir(env), layout: 'flat' },
    { name: 'copilot', dir: copilotSessionsDir(env), layout: 'flat' },
  ];
}

/**
 * Claude Code names each project directory after its absolute path with the
 * separators flattened: `/Volumes/NIT-SSD/Development/Gems` becomes
 * `-Volumes-NIT-SSD-Development-Gems`. Dots flatten too, which is why
 * `.../Class-mode/.claude/worktrees/...` lands as `...Class-mode--claude-worktrees-...`.
 *
 * This is a fast path only. `locateTranscript` falls back to a scan, because the
 * naming rule is Claude Code's to change and session ids are unique on their own.
 */
export function projectSlug(cwd) {
  if (typeof cwd !== 'string' || cwd.length === 0) return null;
  return cwd.replace(/[/.]/g, '-');
}

export function isSafeSessionId(sessionId) {
  return typeof sessionId === 'string' && /^[a-zA-Z0-9_-]+$/.test(sessionId);
}

/**
 * `existsSync` also matches directories, and a directory named `<id>.jsonl` is not a
 * transcript — it would otherwise be recorded with the directory's own size.
 */
function isFile(path) {
  try {
    return statSync(path).isFile();
  } catch {
    return false;
  }
}

function isDir(path) {
  try {
    return statSync(path).isDirectory();
  } catch {
    return false;
  }
}

/**
 * Find `<session_id>.jsonl`. Tries the slug derived from cwd first, then scans every
 * project directory — a session can be recorded under a different project than the
 * cwd we were handed (worktrees, `cd` mid-session).
 */
export function locateTranscript(sessionId, cwd, projectsDir) {
  if (!isSafeSessionId(sessionId)) return null;
  if (!existsSync(projectsDir)) return null;

  const filename = `${sessionId}.jsonl`;

  const slug = projectSlug(cwd);
  if (slug) {
    const fastPath = join(projectsDir, slug, filename);
    if (isFile(fastPath)) return fastPath;
  }

  let entries;
  try {
    entries = readdirSync(projectsDir, { withFileTypes: true });
  } catch {
    return null;
  }

  for (const entry of entries) {
    if (!entry.isDirectory()) continue;
    const candidate = join(projectsDir, entry.name, filename);
    if (isFile(candidate)) return candidate;
  }

  // Also check if it's flat in the root directory (for Codex/Cursor)
  const flatPath = join(projectsDir, filename);
  if (isFile(flatPath)) return flatPath;

  // Check Antigravity (agy) nested structure
  const agyFull = join(projectsDir, sessionId, '.system_generated', 'logs', 'transcript_full.jsonl');
  if (isFile(agyFull)) return agyFull;

  const agyTrunc = join(projectsDir, sessionId, '.system_generated', 'logs', 'transcript.jsonl');
  if (isFile(agyTrunc)) return agyTrunc;

  return null;
}

/**
 * List every capturable session in one source directory, tagged with its source. Returns
 * `{ sessionId, cwd, transcriptPath, source }` entries. Best-effort by design: a directory that
 * does not exist, or a layout that does not match, yields nothing rather than throwing, so a
 * scan across seven CLIs is never derailed by one that is not installed.
 *
 * `cwd` is left null here — the enumerator does not know the working directory a session ran in,
 * and `buildRecord` derives `project_slug` from it, so the scan-all path simply records neither.
 * The single-session hook path still carries cwd through, so nothing is lost there.
 */
function enumerateSource(source) {
  const { name, dir, layout } = source;
  const found = [];
  if (!isDir(dir)) return found;

  const push = (sessionId, transcriptPath) => {
    if (isSafeSessionId(sessionId) && isFile(transcriptPath)) {
      found.push({ sessionId, cwd: null, transcriptPath, source: name });
    }
  };

  let entries;
  try {
    entries = readdirSync(dir, { withFileTypes: true });
  } catch {
    return found;
  }

  for (const entry of entries) {
    if (layout === 'agy') {
      if (!entry.isDirectory()) continue;
      const full = join(dir, entry.name, '.system_generated', 'logs', 'transcript_full.jsonl');
      const trunc = join(dir, entry.name, '.system_generated', 'logs', 'transcript.jsonl');
      if (isFile(full)) push(entry.name, full);
      else if (isFile(trunc)) push(entry.name, trunc);
      continue;
    }

    // `flat` and `claude` both carry `<sessionId>.jsonl` files; claude additionally nests them
    // one level under a project-slug directory.
    if (entry.isFile() && entry.name.endsWith('.jsonl')) {
      push(basename(entry.name, '.jsonl'), join(dir, entry.name));
    } else if (layout === 'claude' && entry.isDirectory()) {
      let inner;
      try {
        inner = readdirSync(join(dir, entry.name), { withFileTypes: true });
      } catch {
        continue;
      }
      for (const f of inner) {
        if (f.isFile() && f.name.endsWith('.jsonl')) {
          push(basename(f.name, '.jsonl'), join(dir, entry.name, f.name));
        }
      }
    }
  }

  return found;
}

/**
 * Every capturable session across every source, in `sources()` priority order. A session id that
 * appears under more than one source is kept only for the first source that has it, so a scan
 * never records the same session twice under two different CLIs.
 */
export function enumerateSessions(env = process.env) {
  const seen = new Set();
  const all = [];
  for (const source of sources(env)) {
    for (const entry of enumerateSource(source)) {
      if (seen.has(entry.sessionId)) continue;
      seen.add(entry.sessionId);
      all.push(entry);
    }
  }
  return all;
}

/**
 * Count newlines and bytes in ONE pass. Reading the size separately from the content
 * lets the two disagree when the transcript grows mid-capture, which would publish a
 * byte count and a line count describing different versions of the same session.
 *
 * `indexOf` is a native scan; a per-byte JS loop over a large transcript is slow enough
 * to risk the hook's timeout, which kills the process before its own error handling runs.
 */
export function measureTranscript(path) {
  return new Promise((resolve) => {
    let lines = 0;
    let bytes = 0;
    const stream = createReadStream(path);
    stream.on('data', (chunk) => {
      bytes += chunk.length;
      let at = chunk.indexOf(0x0a);
      while (at !== -1) {
        lines += 1;
        at = chunk.indexOf(0x0a, at + 1);
      }
    });
    stream.on('error', () => resolve(null));
    stream.on('end', () => resolve({ bytes, lines }));
  });
}

export function parseHookInput(raw) {
  if (typeof raw !== 'string' || raw.trim().length === 0) return null;
  try {
    const parsed = JSON.parse(raw);
    return parsed && typeof parsed === 'object' ? parsed : null;
  } catch {
    return null;
  }
}

/**
 * @param {object} args
 * @param {string} args.sessionId
 * @param {string | null} [args.cwd]
 * @param {string} args.transcriptPath
 * @param {number} args.bytes
 * @param {number | null} args.lines
 * @param {string} args.capturedAt
 * @param {import('./extract.mjs').SessionMetrics | null} [args.metrics]
 * @param {string} [args.source]
 */
export function buildRecord({ sessionId, cwd, transcriptPath, bytes, lines, capturedAt, metrics = null, source = 'claude-code' }) {
  return {
    schema: SCHEMA_VERSION,
    source,
    session_id: sessionId,
    cwd: cwd ?? null,
    project_slug: projectSlug(cwd),
    transcript_path: transcriptPath,
    captured_at: capturedAt,
    bytes,
    lines,
    metrics,
  };
}

/**
 * Read at most the last `DEDUPE_TAIL_BYTES` of the store. This runs on every session
 * end and the store only grows, so reading the whole file would make each capture
 * slower than the last. Duplicate SessionEnd events arrive seconds apart, so the tail
 * is where they always are.
 */
function readStoreTail(storePath) {
  let fd;
  try {
    const size = statSync(storePath).size;
    const length = Math.min(size, DEDUPE_TAIL_BYTES);
    if (length === 0) return '';
    const buffer = Buffer.alloc(length);
    fd = openSync(storePath, 'r');
    readSync(fd, buffer, 0, length, size - length);
    return buffer.toString('utf8');
  } catch {
    return '';
  } finally {
    if (fd !== undefined) {
      try {
        closeSync(fd);
      } catch {
        // Nothing useful to do if the descriptor is already gone.
      }
    }
  }
}

/**
 * Append unless an identical (session_id, bytes) record is already stored. SessionEnd
 * can fire more than once for a session that gets resumed, and a resumed session ends
 * larger than it did before — so a same-id-bigger-bytes record is a real update worth
 * keeping. Readers take the last record per session_id.
 */
export function appendRecord(storePath, record) {
  if (existsSync(storePath)) {
    const existing = readStoreTail(storePath);
    for (const line of existing.split('\n')) {
      if (!line.trim()) continue;
      try {
        const prior = JSON.parse(line);
        if (prior.session_id === record.session_id && prior.bytes === record.bytes) {
          return false;
        }
      } catch {
        // A corrupt line is not a reason to drop a good capture.
      }
    }
  }
  appendFileSync(storePath, `${JSON.stringify(record)}\n`, { mode: OWNER_ONLY_FILE });
  return true;
}

export function diagnose(home, message) {
  try {
    const path = join(home, 'capture.log');
    const line = `${new Date().toISOString()} ${message}\n`;
    if (existsSync(path)) {
      const lines = readFileSync(path, 'utf8').split('\n').filter(Boolean);
      if (lines.length >= MAX_DIAGNOSTIC_LINES) {
        writeFileSync(path, `${lines.slice(-(MAX_DIAGNOSTIC_LINES - 1)).join('\n')}\n${line}`, { mode: OWNER_ONLY_FILE });
        return;
      }
    }
    appendFileSync(path, line, { mode: OWNER_ONLY_FILE });
  } catch {
    // Diagnostics are best-effort by definition.
  }
}

/**
 * The outcome of a capture: on success `record` is present, on a skip `reason` is. Both are
 * optional so the two shapes form one type the caller can read `.record` off after checking
 * `.captured` — without this the reason-only literals would drop `record` from the union.
 * @typedef {{ captured: boolean, record?: ReturnType<typeof buildRecord>, reason?: string }} CaptureResult
 */

/**
 * Turn one already-located transcript into a stored record. Shared by the single-session hook and
 * the scan-all path so the size cap, the extraction pass, the dedupe key and the store's `0600`
 * permissions are defined once. Returns `{ captured, record }` or `{ captured:false, reason }`.
 * @returns {Promise<CaptureResult>}
 */
export async function captureTranscript({
  sessionId,
  cwd = null,
  transcriptPath,
  source,
  home,
  now = () => new Date().toISOString(),
  maxLineCountBytes = MAX_LINE_COUNT_BYTES,
}) {
  let statedSize = null;
  try {
    statedSize = statSync(transcriptPath).size;
  } catch {
    diagnose(home, `skipped: transcript unreadable for ${sessionId}`);
    return { captured: false, reason: 'unreadable' };
  }

  // Past the cap, record the session from stat alone rather than walking a huge file
  // and risking the hook timeout. A record without a line count still beats no record.
  let bytes = statedSize;
  let lines = null;
  let metrics = null;
  if (statedSize <= maxLineCountBytes) {
    const measured = await measureTranscript(transcriptPath);
    if (measured) {
      bytes = measured.bytes;
      lines = measured.lines;
    }

    // Extraction is a second pass over the same file. Measured at ~5ms per MB, so even at
    // the 32 MB cap it is a fraction of the hook's 10s budget — and keeping the passes
    // separate is what lets `bytes` stay the single number the dedupe key is built on.
    try {
      metrics = await extractTranscript(transcriptPath, source);
    } catch {
      // Extraction is the enhancement; the pointer record is the floor. A session that
      // cannot be parsed is still a session that happened.
      metrics = null;
    }

    if (!metrics) diagnose(home, `no metrics extracted for ${sessionId}; stored pointer only`);
  } else {
    diagnose(home, `transcript over cap for ${sessionId}; stored pointer only`);
  }

  const record = buildRecord({ sessionId, cwd, transcriptPath, bytes, lines, capturedAt: now(), metrics, source });
  const written = appendRecord(join(home, 'sessions.jsonl'), record);

  if (!written) {
    diagnose(home, `duplicate: ${sessionId} at ${bytes} bytes already stored`);
  }

  return { captured: written, record };
}

/**
 * Capture one session located from a `{session_id, cwd}` payload — the shape the Claude Code
 * `SessionEnd` hook pipes on stdin. `conversation_id` is accepted as an alias so other harnesses
 * that name the field differently still work.
 * @returns {Promise<CaptureResult>}
 */
export async function captureOne({
  raw,
  env = process.env,
  now = () => new Date().toISOString(),
  maxLineCountBytes = MAX_LINE_COUNT_BYTES,
}) {
  const home = gemsHome(env);
  mkdirSync(home, { recursive: true, mode: OWNER_ONLY_DIR });

  const input = parseHookInput(raw);
  const sessionId = input?.session_id ?? input?.conversation_id ?? null;

  if (!isSafeSessionId(sessionId)) {
    diagnose(home, 'skipped: no usable session id on stdin');
    return { captured: false, reason: 'no-session-id' };
  }

  const cwd = typeof input.cwd === 'string' ? input.cwd : null;

  let transcriptPath = null;
  let detectedSource = null;
  for (const s of sources(env)) {
    transcriptPath = locateTranscript(sessionId, cwd, s.dir);
    if (transcriptPath) {
      detectedSource = s.name;
      break;
    }
  }

  if (!transcriptPath) {
    diagnose(home, `skipped: no transcript found for ${sessionId}`);
    return { captured: false, reason: 'no-transcript' };
  }

  return captureTranscript({
    sessionId,
    cwd,
    transcriptPath,
    source: detectedSource,
    home,
    now,
    maxLineCountBytes,
  });
}

/**
 * Scan every known CLI's transcript directory and capture whatever is new. This is what makes
 * Gems work on a CLI that has no `SessionEnd`-style hook: the transcripts are already on disk, so
 * we read them on demand rather than waiting for a lifecycle event. Idempotent — the dedupe in
 * `appendRecord` means running it repeatedly (e.g. before every `/gems`) only appends genuinely
 * new or grown sessions.
 *
 * Returns `{ captured, skipped, sources }` where `sources` counts new captures per CLI.
 */
export async function captureAll({
  env = process.env,
  now = () => new Date().toISOString(),
  maxLineCountBytes = MAX_LINE_COUNT_BYTES,
} = {}) {
  const home = gemsHome(env);
  mkdirSync(home, { recursive: true, mode: OWNER_ONLY_DIR });

  let captured = 0;
  let skipped = 0;
  const perSource = {};

  for (const entry of enumerateSessions(env)) {
    let result;
    try {
      result = await captureTranscript({
        sessionId: entry.sessionId,
        cwd: entry.cwd,
        transcriptPath: entry.transcriptPath,
        source: entry.source,
        home,
        now,
        maxLineCountBytes,
      });
    } catch (err) {
      // One unreadable transcript must not abort a sweep across everything else.
      diagnose(home, `error scanning ${entry.sessionId}: ${err?.message ?? String(err)}`);
      continue;
    }
    if (result.captured) {
      captured += 1;
      perSource[entry.source] = (perSource[entry.source] ?? 0) + 1;
    } else {
      skipped += 1;
    }
  }

  return { captured, skipped, sources: perSource };
}
